// =============================================================================
// Feather Icons - Inline SVG (CSP-safe for Chrome Extensions)
// Selected icons from https://feathericons.com/
// =============================================================================
...
const FeatherIcons = {
  ...
  'pause-circle': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="10" y1="15" x2="10" y2="9"></line><line x1="14" y1="15" x2="14" y2="9"></line></svg>',
  ...
  'volume-1': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path></svg>',
...
// Helper function to replace data-feather attributes with actual SVG
window.featherReplace = function() {
  document.querySelectorAll('[data-feather]').forEach(el => {
    const iconName = el.getAttribute('data-feather');
    const svg = FeatherIcons[iconName];
    if (svg) {
      // Preserve existing styles
      const width = el.style.width || '24px';
      const height = el.style.height || '24px';
      ...
      if (className) svgEl.className = className;
      
      // Copy other styles
      Array.from(el.style).forEach(prop => {
        if (prop !== 'width' && prop !== 'height') {
          svgEl.style[prop] = el.style[prop];
        }
      });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => window.featherReplace());
} else {
